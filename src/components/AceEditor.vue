  ...
  <div style="max-width: 100vw; overflow-x: auto;">
    <div class="editor-top">
      <div 
        ...
        @drop="onDrop(index)"
        :style="{ 
          fontWeight: file.id === props.currentFile?.id ? 'bold' : 'normal', 
          color: file.id === props.currentFile?.id ? 'white' : 'lightgrey', 
          backgroundColor: file.id === props.currentFile?.id ? '#212121' : '#282922',
          margin: 0 
        }"
      >
        <span @click="emit('update:currentFile', file)">{{ file.fileName }}</span>
        <img src="@/assets/crossicon.png" @click.stop="emit('closeFile', file.id)" class="cross"/>
      </div>
    </div>
    <div>
      <VAceEditor        
